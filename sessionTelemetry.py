    ...
    def initialize_animation_objects(self, drivers_data, ax_track, ax_speed, ax_throttle, ax_brake, ax_delta, track_x, track_y):
        ...
        for i, (driver, data) in enumerate(drivers_data.items()):
            ...
            if driver != reference_driver:
                ...
                distances_normalized = distances - distances[0]
                ref_distances_normalized = ref_distances - ref_distances[0]
                
                lap_percentage = (distances_normalized / distances_normalized[-1]) * 100
                ref_lap_percentage = (ref_distances_normalized / ref_distances_normalized[-1]) * 100
                
                ref_elapsed_interp = np.interp(lap_percentage, ref_lap_percentage, ref_elapsed)
                deltas = driver_elapsed - ref_elapsed_interp
